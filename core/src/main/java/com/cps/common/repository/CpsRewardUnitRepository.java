...
public interface CpsRewardUnitRepository extends JpaRepository<CpsRewardUnitEntity, String> {
    ...
    @Query("SELECT new com.cps.common.dto.UnitListDto(C.rewardUnitNum, C.cnt, C.stockCnt, C.totalPrice, C.productName, C.rewardCnt, C.status, C.regDay) FROM CpsRewardUnitEntity C WHERE C.userId = :userId AND C.merchantId =  :merchantId AND C.affliateId = :affliateId AND (:regYm IS NULL OR :regYm = 0 OR C.regYm = :regYm) AND C.status IN :status ")
    ...
    @Query("SELECT new com.cps.common.dto.UnitListDto(C.rewardUnitNum, C.cnt, C.stockCnt, C.totalPrice, C.productName, C.rewardCnt, C.status, C.regDay) FROM CpsRewardUnitEntity C WHERE C.userId = :userId AND C.merchantId =  :merchantId AND C.affliateId = :affliateId AND C.status = 200 ORDER BY C.regDate ASC")
    ...
    @Query("SELECT new com.cps.common.dto.UnitDto(sum(C.cnt), sum(C.stockCnt)) FROM CpsRewardUnitEntity C WHERE C.userId = :userId AND C.merchantId = :merchantId AND C.affliateId = :affliateId AND C.status = 200")
