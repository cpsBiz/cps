...
public interface CpsMemberCommissionRepository extends JpaRepository<CpsRewardEntity, String> {
    @Query(value = "SELECT A.USER_ID, A.REG_DAY, A.REG_YM, B.CAMPAIGN_NAME, A.PRODUCT_NAME, A.PRODUCT_PRICE, A.USER_COMMISSION, A.PRODUCT_CNT, A.MERCHANT_ID, A.STATUS, B.COMMISSION_PAYMENT_STANDARD " +
            "FROM CPS_REWARD A " +
    ...
    @Query(value = "SELECT new com.cps.cpsService.dto.CpsMemberRewardUnitDto ( " +
            "COALESCE(SUM(CASE WHEN A.status = 310 THEN A.cnt ELSE 0 END), 0), " +
            "COALESCE(SUM(CASE WHEN A.status IN (100, 200) THEN A.cnt ELSE 0 END), 0)) " +
