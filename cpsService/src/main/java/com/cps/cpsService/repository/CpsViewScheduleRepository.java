...
public interface CpsViewScheduleRepository extends JpaRepository<CpsViewEntity, String> {
    @Query(value = "INSERT INTO SUMMARY_HOUR (REG_DAY, REG_YM, REG_HOUR, CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, CNT, TYPE) " +
            "SELECT REG_DAY, SUBSTR(REG_DAY,1,6) AS REG_YM,  REG_HOUR, " +
            "       CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, COUNT(1) AS CNT, TYPE  " +
            "  FROM CPS_VIEW " +
            " WHERE REG_DAY = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :dayMinute MINUTE), '%Y%m%d') " +
            "   AND REG_HOUR = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :hourMinute MINUTE), '%H') " +
            " GROUP BY CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS " +
            "    ON DUPLICATE KEY UPDATE CNT = VALUES(CNT), MOD_DATE = NOW() ", nativeQuery = true)
    void insertSummaryViewHour(@Param("dayMinute") int dayMinute, @Param("hourMinute") int hourMinute);


    @Query(value = "INSERT INTO SUMMARY_HOUR (REG_DAY, REG_YM, REG_HOUR, CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, CLICK_CNT, TYPE) " +
            "SELECT REG_DAY, SUBSTR(REG_DAY,1,6) AS REG_YM,  REG_HOUR, " +
            "       CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, COUNT(1) AS CLICK_CNT, TYPE  " +
            "  FROM CPS_CLICK " +
            " WHERE REG_DAY = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :dayMinute MINUTE), '%Y%m%d') " +
            "   AND REG_HOUR = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :hourMinute MINUTE), '%H') " +
            " GROUP BY CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS " +
            "    ON DUPLICATE KEY UPDATE CLICK_CNT = VALUES(CLICK_CNT), MOD_DATE = NOW() ", nativeQuery = true)
    void insertSummaryClickHour(@Param("dayMinute") int dayMinute, @Param("hourMinute") int hourMinute);

    @Query(value = "INSERT INTO SUMMARY_HOUR (REG_DAY, REG_YM, REG_HOUR, CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS  " +
            ...
            "     , A.CAMPAIGN_NUM, A.MERCHANT_ID, A.AGENCY_ID, A.AFFLIATE_ID, A.ZONE_ID, A.SITE, A.OS  " +
            ...
            "     , A.TYPE " +
            "  FROM ( " +
            "        SELECT REG_DAY, REG_HOUR, CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS,  " +
            "               COUNT(1) AS REWARD_CNT," +
            "               CASE WHEN STATUS IN (100, 210) THEN COUNT(1) ELSE 0 END AS CONFIRM_REWARD_CNT," +
            "               CASE WHEN STATUS = 310 THEN COUNT(1) ELSE 0 END AS CANCEL_REWARD_CNT, " +
            "               SUM(PRODUCT_PRICE) AS PRODUCT_PRICE," +
            "               CASE WHEN STATUS IN (100, 210) THEN SUM(PRODUCT_PRICE) ELSE 0 END AS CONFIRM_PRODUCT_PRICE," +
            "               CASE WHEN STATUS = 310 THEN SUM(PRODUCT_PRICE) ELSE 0 END AS CANCEL_PRODUCT_PRICE, " +
            "               SUM(COMMISSION) AS COMMISSION," +
            "               CASE WHEN STATUS IN (100, 210) THEN SUM(COMMISSION) ELSE 0 END AS COMFIRM_COMMISSION," +
            "               CASE WHEN STATUS = 310 THEN SUM(COMMISSION) ELSE 0 END AS CANCEL_COMMISSION, " +
            "               SUM(COMMISSION_PROFIT) AS COMMISSION_PROFIT, " +
            "               CASE WHEN STATUS IN (100, 210) THEN SUM(COMMISSION_PROFIT) ELSE 0 END AS COMFIRM_COMMISSION_PROFIT," +
            ...
            "               CASE WHEN STATUS IN (100, 210) THEN SUM(AFFLIATE_COMMISSION) ELSE 0 END AS COMFIRM_AFFLIATE_COMMISSION," +
            ...
            "               CASE WHEN STATUS IN (100, 210) THEN SUM(USER_COMMISSION) ELSE 0 END AS COMFIRM_USER_COMMISSION, " +
            "               CASE WHEN STATUS = 310 THEN SUM(USER_COMMISSION) ELSE 0 END AS CANCEL_USER_COMMISSION, " +
            "               TYPE" +
            "          FROM CPS_REWARD " +
            "         WHERE REG_DAY = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :dayMinute MINUTE), '%Y%m%d') " +
            "           AND REG_HOUR = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :hourMinute MINUTE), '%H') " +
            "           GROUP BY CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, STATUS  )A " +
            " GROUP BY A.REG_DAY, A.REG_HOUR, A.CAMPAIGN_NUM, A.MERCHANT_ID, A.AGENCY_ID, A.AFFLIATE_ID, A.ZONE_ID, A.SITE, A.OS" +
            ...
            "     ,  USER_COMMISSION = VALUES(USER_COMMISSION), COMFIRM_USER_COMMISSION = VALUES(COMFIRM_USER_COMMISSION), CANCEL_USER_COMMISSION = VALUES(CANCEL_USER_COMMISSION), MOD_DATE = NOW()  "
                  , nativeQuery = true)
    void insertSummaryRewardHour(@Param("dayMinute") int dayMinute, @Param("hourMinute") int hourMinute);

    @Query(value = "INSERT INTO SUMMARY_DAY (REG_DAY, REG_YM, CAMPAIGN_NUM, MERCHANT_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS " +
            ...
            "       A.CAMPAIGN_NUM, A.MERCHANT_ID, A.AGENCY_ID, A.AFFLIATE_ID, A.ZONE_ID, A.SITE, A.OS , SUM(A.CNT) AS CNT, SUM(A.CLICK_CNT) AS CLICK_CNT, " +
            "       SUM(A.REWARD_CNT) AS REWARD_CNT, SUM(A.CONFIRM_REWARD_CNT) AS CONFIRM_REWARD_CNT,  SUM(A.CANCEL_REWARD_CNT) AS CANCEL_REWARD_CNT, " +
            "       SUM(A.PRODUCT_PRICE) AS PRODUCT_PRICE, SUM(A.CONFIRM_PRODUCT_PRICE) AS CONFIRM_PRODUCT_PRICE,  SUM(A.CANCEL_PRODUCT_PRICE) AS CANCEL_PRODUCT_PRICE, " +
            "       SUM(A.COMMISSION) AS COMMISSION, SUM(A.COMFIRM_COMMISSION) AS COMFIRM_COMMISSION,  SUM(A.CANCEL_COMMISSION) AS CANCEL_COMMISSION, " +
            "       SUM(A.COMMISSION_PROFIT) AS COMMISSION_PROFIT, SUM(A.COMFIRM_COMMISSION_PROFIT) AS COMFIRM_COMMISSION_PROFIT,  SUM(A.CANCEL_COMMISSION_PROFIT) AS CANCEL_COMMISSION_PROFIT,  " +
            ...
            "       (SELECT C.MEMBER_NAME FROM CPS_MEMBER C WHERE C.MEMBER_ID = A.MERCHANT_ID AND C.TYPE = 'MEMBER') AS MEMBER_NAME, " +
            ...
            "       IFNULL((SELECT C.TYPE FROM CPS_MEMBER C WHERE C.MEMBER_ID = A.MERCHANT_ID),'NON_TYPE') AS TYPE " +
            "  FROM SUMMARY_HOUR A" +
            " WHERE REG_DAY = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :dayMinute MINUTE), '%Y%m%d') " +
            " GROUP BY A.CAMPAIGN_NUM, A.MERCHANT_ID, A.AGENCY_ID, A.AFFLIATE_ID, A.ZONE_ID, A.SITE, A.OS " +
    ...
    void insertSummaryViewDay(@Param("dayMinute") int dayMinute);
}
