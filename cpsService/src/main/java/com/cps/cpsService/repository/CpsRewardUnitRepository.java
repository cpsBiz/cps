...
public interface CpsRewardUnitRepository extends JpaRepository<CpsRewardUnitEntity, String> {
    ...
    @Query("SELECT new com.cps.cpsService.dto.UnitListDto(C.cnt, C.stockCnt, C.totalPrice, C.productName, C.rewardCnt, C.status, C.regDay) FROM CpsRewardUnitEntity C WHERE C.userId = :userId AND C.merchantId =  :merchantId AND C.affliateId = :affliateId AND (:regYm IS NULL OR :regYm = 0 OR C.regYm = :regYm) AND (:status IS NULL OR :status = 0 OR C.status = :status) ")
    ...
    @Query("SELECT new com.cps.cpsService.dto.UnitDto(sum(C.cnt), sum(C.stockCnt)) FROM CpsRewardUnitEntity C WHERE C.userId = :userId AND C.merchantId = :merchantId AND C.affliateId = :affliateId AND C.status = 210")
