...
public interface CpsViewScheduleRepository extends JpaRepository<CpsViewEntity, String> {
    ...
    @Query(value = "INSERT INTO SUMMARY_DAY (REG_DAY, REG_YM, CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS " +
            ...
            "       (SELECT C.MEMBER_NAME FROM CPS_USER C WHERE C.MEMBER_ID = A.MEMBER_ID AND C.TYPE = 'MEMBER') AS MEMBER_NAME, " +
            "       (SELECT C.MEMBER_NAME FROM CPS_USER C WHERE C.MEMBER_ID = A.AGENCY_ID AND C.TYPE = 'AGENCY') AS AGENCY_NAME, " +
            "       (SELECT C.MEMBER_NAME FROM CPS_USER C WHERE C.MEMBER_ID = A.AFFLIATE_ID AND C.TYPE = 'AFFLIATE') AS AFFLIATE_NAME, " +
