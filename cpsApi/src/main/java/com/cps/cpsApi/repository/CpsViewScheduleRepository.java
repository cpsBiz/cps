...
public interface CpsViewScheduleRepository extends JpaRepository<CpsViewEntity, String> {
    @Query(value = "INSERT INTO SUMMARY_HOUR (REG_DAY, REG_YM, REG_HOUR, CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, CNT) " +
            "SELECT REG_DAY, SUBSTR(REG_DAY,1,6) AS REG_YM,  REG_HOUR, " +
            "       CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, COUNT(1) AS CNT  " +
            "  FROM CPS_VIEW " +
    ...
    @Query(value = "INSERT INTO SUMMARY_HOUR (REG_DAY, REG_YM, REG_HOUR, CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, CLICK_CNT) " +
            "SELECT REG_DAY, SUBSTR(REG_DAY,1,6) AS REG_YM,  REG_HOUR, " +
            "       CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, COUNT(1) AS CLICK_CNT  " +
            "  FROM CPS_CLICK " +
            " WHERE REG_DAY = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :minutes MINUTE), '%Y%m%d') " +
            "   AND REG_HOUR = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL :minutes MINUTE), '%H') " +
            " GROUP BY CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS " +
    ...
    @Query(value = "INSERT INTO SUMMARY_DAY (REG_DAY, REG_YM, CAMPAIGN_NUM, MEMBER_ID, AGENCY_ID, AFFLIATE_ID, ZONE_ID, SITE, OS, CLICK_CNT, TYPE) " +
            ...
            "       (SELECT C.TYPE FROM CPS_USER C WHERE C.MEMBER_ID = A.MEMBER_ID) AS TYPE " +
            "  FROM SUMMARY_HOUR A" +
