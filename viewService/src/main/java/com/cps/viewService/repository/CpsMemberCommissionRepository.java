...
import com.cps.viewService.entity.CpsRewardEntity;
...
public interface CpsMemberCommissionRepository extends JpaRepository<CpsRewardEntity, String> {
    ...
    @Query(value = "SELECT new com.cps.viewService.dto.CpsMemberCommissionDto ( " +
            "SUM(CASE WHEN A.status = 310 THEN A.userCommission ELSE 0 END) AS userCommission, " +
            "SUM(CASE WHEN A.status IN (100, 200) THEN A.userCommission ELSE 0 END) AS expectedUserCommission) " +
