...
$(document).ready(function() {
    var with_aos_zone = ["10886105", "10886106", "10886107", "10886105", "10886106", "10886107"]; //aos용 지면
    var with_ios_zone = ["10886102", "10886103", "10886104", "10886102", "10886103", "10886104"]; //ios용 지면
...
//        }
    ...
    // iframe이 로드될 때의 이벤트 핸들러 등록
    /*$('#mobwith').on('load', function() {
        console.log('iframe 로드 완료');
        var iframeContents = $('#mobwith').contents();
        var inputElement = iframeContents.find('#lastMoment');

        var resVal = inputElement.val();
...
function callGetApi(index){
    ...
    //var with_domain = "https://dev.mobwithad.com/api/banner/app/vp/v1/paybooc?zone=";
...
function callCoupangApi(){
    ...
    //localStorage.setItem("ad_coupang_" + adid + "_" + todayDate(), "Y"); //쿠팡 광고 노출여부 등록
    coupang_yn ="Y";
    //비회원인 경우 adid 임의처리
    if(adid == ""){
        adid = "00af6187-5ee4-4ed6-877d-ec34f75857f9";
    }
    var call_url = "/view/coupang"+"?adid="+adid+"&os="+os;
    $("#mobwith").attr("src", call_url);

    var param = { "ad_id" : zone , "point" : coupang_point };
    console.log(param);
    reqNative("ad_info", param); // 앱 함수 호출
}
...
function reqNative(event, data) {
    ...
    try {
        ...
        console.error("reqMoaGroup error ::", err);
    }
}
